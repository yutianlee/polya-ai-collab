...
\hypersetup{
  ...
  pdfsubject={Main proof manuscript},
...
\fancyhead[R]{\small Main proof manuscript}
\fancyfoot[C]{\thepage}
...
contracts for the finite interval computations are stated in this paper.
The complete executable replay sources are collected in the accompanying
certification supplement.  The imported background and analytic results
...
\[
  ...
  \qquad \Lambda\ge0.
\]
This is a computer-assisted proof.  It separates variables, converts the
radial spectrum into a strict multiplicity-weighted phase count, and combines
analytic endpoint estimates,
finite angular and radial mode inventories, and a rigorously certified final
compact window using exact-rational checks and outward Arb interval
certificates; the complete replay sources are provided in the accompanying
certification supplement.  We also give an elementary proof that no genuine spherical
...
Supplement~S1, \emph{Standalone exact-rational analytic ledgers}.
...
Supplement~S1 maps every printed moving numerator and fixed margin to its wall
and cap.
\end{proof}
...
Supplement~S2, \emph{Executable interval certificates}.  Thus the finite
predicates just described prove the lemma.
\end{proof}
...
\]
The complete executable is printed in Supplement~S2.
\end{proof}
...
outside \(\mathcal D_{20}\).  The exact classifier printed in Supplement~S1
enumerates all
...
\item the complete replay sources are printed verbatim in Supplements~S1
and~S2.  They read no input files, use only exact rational data and outward
Arb balls, and exit nonzero on the first failed proof predicate.
\end{enumerate}
...
\small
\begin{tabular}{l|l}
exact-rational ledger&\(611\) checks; canonical registry; PASS\\
...
\end{tabular}
\end{center}
The exact-rational registry contains (488) substantive checks, (65)
bookkeeping identities, and (58) supplemental checks.  Its canonical
registry SHA--256 is
\[
\begin{gathered}
\mathtt{afd7e054f52aa9a3992fc8ef16d3e755}\\[-2pt]
\mathtt{1586c76bfb589053714f5f55a36792f1}.
\end{gathered}
\]
The SHA--256 digests of the three source listings are
\[
\begin{array}{c|c}
\text{exact-rational source}&
\begin{gathered}
\mathtt{41691116e0e3e5453b9b209acb93ff78}\\[-2pt]
...
in Supplement~S2; they are not additional mathematical assumptions.  The
recorded counts, source digests, certificate digests, and replay results make
the regeneration byte-identifiable.
...
\begin{enumerate}
  ...
  \item outward midpoint-radius interval arithmetic as implemented by Arb
        ...
        and deterministic regeneration algorithm are all stated in the main
        paper, while the complete executable sources are printed in the
        accompanying certification supplement.  Decimal displays are never
        proof predicates.
\end{enumerate}
...
outputs of the supplementary algorithms, not additional mathematical assumptions.
