...
with at least one active radial layer.  First, an exact Stieltjes formula is
given with the strict common-jump convention made explicit.  The negative
mean of every complete angular cell is combined with the single-peaked shape
of $q=-A'$ to obtain continuous block estimates.  A paired radial block
argument then controls the full shifted sample sum, including all common
jumps, by
\[
 \mathcal E_{\rm ang}
...
\section{Complete angular cells and the single peak}
...
\begin{equation}
 ...
 \label{eq:e}
\end{equation}
For $m\geq1$, the strict $m$-cell is
\[
 I_m=(m-1/2,m+1/2],
 \qquad e(r)=m^2-r^2\quad(r\in I_m).
\]
Endpoint values do not affect the following Lebesgue integrals.  The
positive and negative half-cell masses are
\begin{align}
 P_m&:=\int_{m-1/2}^{m}(m^2-r^2)\,dr
     =\frac m4-\frac1{24},
 \label{eq:Pm}\\
 Q_m&:=-\int_m^{m+1/2}(m^2-r^2)\,dr
     =\frac m4+\frac1{24}.
 \label{eq:Qm}
\end{align}
In particular,
\begin{equation}
 \int_{m-1/2}^{m+1/2}e(r)\,dr
 =P_m-Q_m=-\frac1{12}.
 \label{eq:negative-mean}
\end{equation}

\begin{lemma}[Weighted complete-cell bounds]
\label{lem:weighted-cells}
Let $m\geq1$ and suppose the complete cell
$[m-1/2,m+1/2]$ lies in $[0,K]$.
\begin{enumerate}
\item If $m+1/2\leq\rho K$, then
\begin{equation}
 \int_{m-1/2}^{m+1/2}e(r)q(r)\,dr
 \leq-\frac{q(m)}{12}\leq0.
 \label{eq:increasing-cell}
\end{equation}
\item If $m-1/2\geq\rho K$, then
\begin{equation}
 \int_{m-1/2}^{m+1/2}e(r)q(r)\,dr
 \leq P_mq(m-1/2)-Q_mq(m+1/2).
 \label{eq:decreasing-cell}
\end{equation}
\item On the unique cell, if any, whose interior contains $\rho K$,
\begin{equation}
 \int_{m-1/2}^{m+1/2}e(r)q(r)\,dr
 \leq P_m q(\rho K).
 \label{eq:peak-cell}
\end{equation}
\end{enumerate}
The initial partial cell $0\leq r\leq1/2$ contributes nonpositively.
\end{lemma}

\begin{proof}
On the left half of a cell $e\geq0$, and on the right half $e\leq0$.
If $q$ is increasing, then $q(r)\leq q(m)$ on the positive half and
$q(r)\geq q(m)$ on the negative half.  Equations
\eqref{eq:Pm}--\eqref{eq:negative-mean} give
\eqref{eq:increasing-cell}.

If $q$ is decreasing, then $q(r)\leq q(m-1/2)$ on the positive half and
$q(r)\geq q(m+1/2)$ on the negative half.  This gives
\eqref{eq:decreasing-cell}.  On the peak cell, discard the negative half
and use $q\leq q(\rho K)$ on the positive half.  Finally, $M(r)=0$ and
$e(r)=-r^2$ for $0\leq r\leq1/2$.
\end{proof}

Thus the continuous angular defect has a favorable analytic block
structure:
\begin{equation}
 \int_0^T e(R(t))\,dt=\int_0^K e(r)q(r)\,dr.
 \label{eq:continuous-defect}
\end{equation}
All complete cells below the peak are nonpositive, and the decreasing
side is controlled by the endpoint differences in
\eqref{eq:decreasing-cell}.  The difficulty is passing from the integral
in \eqref{eq:continuous-defect} to the shifted samples in
\eqref{eq:Eang}; the next subsection records that passage with all common
jumps included.

\section{Exact common-jump convention}

Let
\begin{equation}
 h(t):=e(R(t)),
 \qquad
 C(t):=\#\{n:0<x_n\leq t,\ x_n<T\},
 \qquad
 p(t):=C(t)-t.
 \label{eq:Euler-data}
\end{equation}
Choose the right-continuous versions of $C,p$, and $h$.  Right continuity
of $h$ is exactly the strict angular convention: when $R(t)=m+1/2$,
the post-jump value uses $M=m$.  The atomic measure $dC$ therefore samples
the correct value even when a radial point $x_n$ is also an angular wall.

\begin{lemma}[Euler identity with common jumps]
\label{lem:common-jump}
With the preceding representatives,
\begin{equation}
 \mathcal E_{\rm ang}
 =\int_0^T h(t)\,dt
  -\int_{(0,T]}p(t^- )\,dh(t).
 \label{eq:common-jump-identity}
\end{equation}
At a common radial--angular wall the second integral uses the left value
$p(t^- )=-3/4$; no extra or omitted wall term is permitted.
\end{lemma}

\begin{proof}
The measure $dC$ has a unit atom at every active $x_n$, so
\[
 \mathcal E_{\rm ang}=\int_{(0,T)}h(t)\,dC(t).
\]
Since $dC=dt+dp$,
\[
 \mathcal E_{\rm ang}=\int_0^T h(t)\,dt+\int_{(0,T]}h(t)\,dp(t).
\]
For right-continuous functions of bounded variation, the product formula
in the form
\[
 \int_{(0,T]}h(t)\,dp(t)
 =[hp]_0^T-\int_{(0,T]}p(t^- )\,dh(t)
\]
already contains the product of simultaneous jumps.  Here $p(0)=0$ and
$h(T)=e(0)=0$, so the boundary term vanishes.  This proves
\eqref{eq:common-jump-identity}.  If $t=x_n$, then
$C(t^-)=n-1$ and hence $p(t^-)=n-1-(n-1/4)=-3/4$, including a common
angular wall.
\end{proof}

Equation \eqref{eq:common-jump-identity} explains why the negative means
in \eqref{eq:negative-mean} cannot be used without a jump audit.  The
second term contains both the continuous rise inside each angular cell and
the downward atom at its strict lower wall.  A direct sign deletion would
be invalid.  The paired estimate below controls the integral and all its
atoms simultaneously.

\section{A paired radial-block estimate}
...
\begin{equation}
 ...
 \label{eq:N-positive}
\end{equation}
so $N\geq1$.  Proposition~\ref{prop:global-E} and
\eqref{eq:structural-implication} reduce the global middle-ratio theorem to
...
The complete-cell bounds, the common-jump formula, and the global estimate
\eqref{eq:global-E-bound} are proved analytically.  The first remaining
step within this module is the smooth scalar inequality
