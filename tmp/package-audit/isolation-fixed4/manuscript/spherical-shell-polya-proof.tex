...
accompanying purely analytic supplement.  Executable replay sources and
interval certificates are retained only as optional regression checks and
are not premises of the proof.  The imported results are explicitly
identified standard Sturm--Liouville and Bessel facts, the published uniform
Bessel-phase enclosure, and the annular phase-difference estimate, each
quoted with its precise scope.
\end{quote}
...
conclusion is a proof premise.  An independent appendix gives an elementary
proof that no genuine spherical shell tiles \(\R^3\) by congruent
...
\]
In the action variables of Part~I of the supplement, let \(R\) be
...
\[
 ...
 =\frac{22}{7}-\pi.
\]
Since \(22/7-333/106=1/742>0\), all four rational numbers are ordered
as asserted.
...
\paragraph{Exact optical reserve ledger.}
...
\midrule
Machin lower certificate above \(333/106\)&
\(3418213/41563593750\)\\
...
and exact-rational inequality.  It uses only the following conventional
external results, each in the scope stated where it enters the proof:
\begin{enumerate}
