...
\begin{proof}
On the inner branch, direct differentiation of \eqref{rs:eq:q} shows that
$q'>0$; on the outer branch $q'<0$.  Both one-sided values at
$z=\rho K$ equal $\theta/\pi$, proving \eqref{rs:eq:q-cap}.

If $z=R(t)>\rho K$, then
\[
 g(t)=\frac{2\pi z}{\arccos(z/K)}.
\]
The quotient on the right increases with $z$, while $z$ decreases with
$t$, so $g$ decreases on $(0,\tau)$.  On the inner branch put
\[
...
\]
shows that $\delta(z)/z$ increases with $z$.  Hence
...
resulting coefficient is positive.  Dropping the positive
