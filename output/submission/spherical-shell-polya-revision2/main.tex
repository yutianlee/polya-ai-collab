...
boundary assignments, and exact constants are stated in this paper and its
accompanying purely analytic supplement.  The imported results are explicitly
identified standard Sturm--Liouville and Bessel facts, the published uniform
Bessel-phase enclosure, and the annular phase-difference estimate, each quoted
...
at the first possible radial frequency and covers every remaining finite
frequency analytically.  An independent appendix gives an elementary proof
that no genuine spherical shell tiles \(\R^3\) by congruent
rigid-motion copies with disjoint interiors, under either exact or
...
\]
Let \(R\) be the decreasing inverse of the shell action
\(A_{\rho,K}\) from \eqref{eq:sc-shell-action}, put \(F=R^2\), and let
\(M(r)\) count the half-integers strictly below \(r\).  The inverse action
gives the exact layer-cake picture
\[
...
\[
 \frac{d}{dy}\left(\frac{y}{\alpha}\right)
 =\frac{\alpha+y/\sqrt{a^2-y^2}}{\alpha^2}>0.
\]
Since \(y=R(t)\) decreases with \(t\), equation
\eqref{thin:eq-g-outer} shows that \(g\) decreases on \(0<t<\tau\).
...
\label{lem:rational-pi-bounds}
One has
\[
 \frac{157}{50}<\frac{333}{106}<\pi<\frac{22}{7},
 \qquad\text{and hence}\qquad \frac1\pi<\frac{106}{333}.
\]
\end{lemma}
...
\[
 ...
 =\frac{22}{7}-\pi.
\]
Since \(22/7-333/106=1/742>0\), all four displayed quantities are ordered
as asserted.
This proves every displayed comparison.
\end{proof}

\subsection{All-frequency optical theorem}

\begin{lemma}[All-frequency optical theorem]
\label{opt:lem-optical}
...
\begin{equation}
 ...
 \label{opt:eq-product-deficit}
\end{equation}
For \(C_D=1382/3125\), the difference
\(H(N,t):=D/\pi^2-C_D(N+t)^2\) increases in \(N\).  Indeed, for
\(N\geq1\) and \(0<t\leq1\),
\[
 \partial_NH
 =(1-2C_D)N+t^2+\frac16-2C_Dt
 \geq1-2C_D+\frac16-C_D^2
 =\frac{5074831}{58593750}>0.
\]
At \(N=1\), one has
\[
 \partial_tH(1,t)=2(t-C_D)(1+t),
\]
so the minimum occurs at \(t=C_D\), where it equals
...
\]
On \(a\leq c/\varepsilon\), using
\(q=106/333>1/\pi\) from Lemma~\ref{lem:rational-pi-bounds} and
\(\varepsilon\leq11/50\), the exact reserve is
\begin{equation}
 C_D-\left(\frac{2cq}{3}+\frac{11}{200}
 +(11/50)^2q^2\right)
 =\frac{39569}{2772225000}>0.
 \label{opt:eq-low-reserve}
\end{equation}
...
\]
Dividing \eqref{opt:eq-radial-deficit} and
\eqref{opt:eq-angular-error} by \(a^2\), then using
Lemma~\ref{lem:rational-pi-bounds}, gives
\[
...
\paragraph{Exact optical reserve table.}
The small margins in the preceding proof are recorded here to make later
constant changes auditable.  Each entry is an exact positive rational
quantity, not a decimal or numerical premise.
\begin{center}
\small
\begin{tabular}{@{}ll@{}}
\toprule
exact check&positive reserve\\
\midrule
Machin lower estimate above \(333/106\)&
\(3418213/41563593750\)\\
monotonicity of \(H\) in \(N\)&\(5074831/58593750\)\\
minimum of the product-deficit function \(H\)&
\(1822532/91552734375\)\\
...
\(39/50\leq\rho<1\), apply Lemma~\ref{opt:lem-optical}.  These cases,
...
\section{Independent non-tiling theorem}
...
spectral reduction, weighted count, regional subtraction, boundary assignment,
and exact-rational inequality.  Its external verification boundary is the
following, with each item used only in the stated scope:
\begin{enumerate}
  \item standard functional-analysis and measure-theory facts, including
        closed-form representation, compact embedding, orthogonal direct
        sums, Fubini--Tonelli, Parseval, and Stieltjes integration by parts;
  \item standard spherical-harmonic decomposition, regular
        Dirichlet Sturm--Liouville theory, direct-sum facts, and the min--max
        principle;
  \item the Bessel equation and standard identities, its Wronskian,
        Nicholson's representation, and the usual small- and large-argument
        formulas collected in \cite{DLMF};
  \item the real-order phase enclosure of \cite{FLPSphase} and the annular
        phase-difference estimates stated as Theorem~5.1 and Lemma~5.2 of
        \cite{FLPSannuli}.
\end{enumerate}

The rational bounds
\(157/50<333/106<\pi<22/7\), including the reciprocal inequality used in
the optical theorem, are proved in Lemma~\ref{lem:rational-pi-bounds} rather
than imported.  Every nonstandard scalar estimate is likewise proved in the
paper or the accompanying \emph{Purely Analytic Supplement}.  No numerical
experiment, floating-point computation, interval certificate, or executable
program is a premise of the proof.
...
and the standard identities and limiting formulas used above,
